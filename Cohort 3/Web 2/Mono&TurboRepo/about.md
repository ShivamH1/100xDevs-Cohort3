Ref - https://petal-estimate-4e9.notion.site/Monorepos-b40e9c44a2ee4fe3badc3883841fc2c0?pvs=18
...
![turbo.json](image-5.png)
The `turbo.json` file is a configuration file for TurboRepo, a build system orchestrator used in monorepos. This JSON file defines various tasks and configurations that TurboRepo will manage. Here's an explanation of its contents:

- **$schema**: Specifies the schema URL for validation, ensuring the configuration adheres to a specified format.

- **ui**: Sets the user interface type for TurboRepo, here defined as "tui".

- **tasks**: This object holds different tasks that TurboRepo can execute. Each task has specific properties and dependencies:

  - **build**:

    - **dependsOn**: This indicates that the `build` task depends on other `build` tasks in the dependency graph.
    - **inputs**: Lists the files or patterns that influence the task. Here, it includes default TurboRepo inputs and any files matching `.env*`.
    - **outputs**: Specifies the files or directories to be generated by this task. It includes everything under `.next/**` but excludes `.next/cache/**`.

  - **lint**:

    - **dependsOn**: Indicates dependency on other `lint` tasks.

  - **check-types**:

    - **dependsOn**: Indicates dependency on other `check-types` tasks.

  - **dev**:
    - **cache**: Set to `false`, meaning this task's results are not cached.
    - **persistent**: Set to `true`, indicating this task runs continuously and does not terminate.

This configuration allows TurboRepo to manage and optimize task execution, leveraging features like caching and parallelization, while respecting task dependencies within a monorepo.
