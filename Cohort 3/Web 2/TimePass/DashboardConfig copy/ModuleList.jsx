...
function ModuleCard({ module, isExpanded, onToggleExpand, onToggleModule, onToggleFeature, onSelectStorage }) {
  const { id, name, icon: Icon, enabled, description, features } = module

  return (
    <Card className={`overflow-hidden transition-all duration-200 ${isExpanded ? "ring-2 ring-primary" : ""}`}>
      <div className="p-6 cursor-pointer" onClick={onToggleExpand}>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className={`p-3 rounded-lg ${enabled ? "bg-primary text-primary-foreground" : "bg-muted"}`}>
...
function FeatureItem({ feature, onToggle, onSelectStorage }) {
  const selectedStorage = feature.storageOptions?.find((option) => option.selected)
  const selectedValue = selectedStorage?.value || ""

  return (
    <div className="flex items-center justify-between p-3 rounded-lg bg-white">
      <div className="flex items-center space-x-3">
        <Checkbox id={feature.id} checked={feature.enabled} onCheckedChange={onToggle} />
        <label htmlFor={feature.id} className="text-sm font-medium">
          {feature.name}
        </label>
      </div>
      {feature.enabled && feature.id === "storage" && feature.storageOptions && (
        <div className="flex-1 ml-8">
          <RadioGroup
            value={selectedValue}
            onValueChange={(value) => onSelectStorage(value)}
            className="flex flex-col space-y-2"
          >
            {feature.storageOptions.map((storage) => (
              <div key={storage.value} className="flex items-center space-x-2">
                <RadioGroupItem value={storage.value} id={storage.value} />
